...
const Navbar = () => {
    ...
    const {user, setUser, navigate, setShowUserLogin, totalCartItems, searchQuery, setSearchQuery}  = useContext(AppContext);
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            <Link to={"/"}>
                <h1 className="text-3xl font-bold text-orange-600">GrabIt</h1>
            </Link>

            <div className="hidden sm:flex items-center gap-8">
                <Link to={"/"}>Home</Link>
                <Link onClick={() => setSearchQuery("")} to={"/products"}>All Products</Link>
                ...
                <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
                    ...
                    <button className="absolute -top-2 -right-3 text-xs text-white bg-indigo-500 w-[18px] h-[18px] rounded-full">{totalCartItems()}</button>
                </div>
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                <Link onClick={() => setSearchQuery("")} to={"/products"} className="block">All Products</Link>
                ...
                {user ? (
                    ...
                    <button onClick={() => setShowUserLogin(true)} className="cursor-pointer px-6 py-2 mt-2 bg-indigo-500 hover:bg-indigo-600 transition text-white rounded-full text-sm">
                    Login
                </button>
                )}
            </div>
